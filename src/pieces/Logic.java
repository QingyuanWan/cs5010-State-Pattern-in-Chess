...
public class Logic {
    ...
    public static boolean isLegal(Piece[][] board, int fr, int fc, int tr, int tc, String toMove) {
        ...
        if (!equalsIgnoreCase(mover.getColor(), toMove)) return false;

        Piece dest = board[tr][tc];
        if (dest != null && equalsIgnoreCase(dest.getColor(), mover.getColor())) return false;

        // Piece movement rules (pseudo-legal)
        if (!pseudoLegal(board, mover, fr, fc, tr, tc)) return false;
    ...
    public static boolean isInCheck(Piece[][] board, String color) {
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                Piece p = board[r][c];
                if (p == null || !equalsIgnoreCase(p.getColor(), enemy)) continue;
                if (pseudoLegal(board, p, r, c, kr, kc)) return true;
            }
        }
        return false;
    }
    ...
    public static boolean isCheckmated(Piece[][] board, String color) {
        ...
        // Try every pseudo-legal move for 'color'; if any resolves check, it's not mate.
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                Piece p = board[r][c];
                if (p == null || !equalsIgnoreCase(p.getColor(), color)) continue;

                for (int tr = 0; tr < 8; tr++) {
                    for (int tc = 0; tc < 8; tc++) {
                        if (r == tr && c == tc) continue;
                        if (!pseudoLegal(board, p, r, c, tr, tc)) continue;
    ...
    private static boolean pseudoLegal(Piece[][] board, Piece mover, int fr, int fc, int tr, int tc) {
        ...
        switch (Character.toUpperCase(mover.symbol())) {
            ...
            case 'P': {
                ...
                // single forward push
                if (dc == 0 && dest == null && dr == dir) return true;
 ...
 */
  ...
  /** Helpers */
    ...
    private static boolean isWhite(Piece p) { return equalsIgnoreCase(p.getColor(), "WHITE"); }
    private static String enemyOf(String color) { return equalsIgnoreCase(color, "WHITE") ? "BLACK" : "WHITE"; }
}
